...
class Author(Library):
    def __init__(self, first_name_author, last_name_author, patronymic_author, date_birth, date_death, *args, **kwargs):
        super().__init__(first_name_author, last_name_author, patronymic_author, *args, **kwargs)
        self.date_birth = date_birth
        self.date_death = date_death

    def __str__(self):
        if self.date_birth[0]:
            return f'Автор {self.first_name_author} {self.last_name_author}.{self.patronymic_author}, \
             годы жизни {self.date_birth} - {self.date_death}'
        else:
            return f'Автор {self.first_name_author} {self.last_name_author}.{self.patronymic_author}, \
             дата смерти {self.date_death}.'

class Production(Library):
    def __init__(self, production_name, date_written, publication_date, publication_city, *args, **kwargs):
        super().__init__(production_name, *args, **kwargs)
    ...
    def __str__(self):
        if self.date_written:
            return f'Произведение {self.production_name}: написано в {self.date_written},\
             опубликовано {self.publication_date} в {self.publication_city}.'
        else:
